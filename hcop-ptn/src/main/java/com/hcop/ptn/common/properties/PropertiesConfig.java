...
import com.hcop.ptn.common.exception.ConfigException;
...
public abstract class PropertiesConfig {
	...
	protected void initProperties(){
		try {
			...
			String confFilePath = currentDir.getAbsolutePath() + File.separator + getConfFile();
	...
	public int getIntConf(String key, String defaultValue) throws ConfigException {
		try {
			return Integer.parseInt(getProperty(key, defaultValue));
		}
		catch (NumberFormatException e) {
			throw new ConfigException("Config '" + key + "' not in int format: '" + getProperty(key) + "'");
		}
	}

	public Boolean getBoolProperty(String key, Boolean defValue) {
		String tmpValue = getProperty(key);
		Boolean result = defValue;
		if ("true".equalsIgnoreCase(tmpValue) || "t".equalsIgnoreCase(tmpValue) || "1".equals(tmpValue))
			result = true;
		else if ("false".equalsIgnoreCase(tmpValue) || "f".equalsIgnoreCase(tmpValue) || "0".equals(tmpValue))
			result = false;
		return result;
	}
	...
	public Boolean getBoolProperty(String key) {
		String tmpValue = getProperty(key);
		if ("true".equalsIgnoreCase(tmpValue) || "1".equals(tmpValue))
			return true;
		else
			return false;
	}
