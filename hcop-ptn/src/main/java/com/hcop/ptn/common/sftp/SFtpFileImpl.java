package com.hcop.ptn.common.sftp;
 ...
 * 通过sftp方式实现文件上传下载
 */
public class SFtpFileImpl implements FtpFile {
    private static Log log = LogFactory.getLog(SFtpFileImpl.class);
    ...
    public boolean uploadFile(String hostName, int port, String userName, String password, String remotePath,
        ...
        try {
            ...
            for (String fileName : localFiles) {
                ...
                if (!file.exists()) {
                    throw new FtpRuntimeException("The file for upload does not exist and the file is:" + fileName);
                }
                if (!file.isFile()) {
                    throw new FtpRuntimeException("The file for upload is a dictionary and the file is:" + fileName);
                }
                sftp.cd(remotePath);
    ...
    public boolean downloadFile(String hostName, int port, String userName, String password, String remoteFileName,
        ...
        try {
            ...
            if (!isDictionary(sftp, remoteFileName)) {
                ...
                throw new FtpRuntimeException("The file download" + remoteFileName + "does not exist!");
            }
        } catch (Exception e) {
            throw new FtpRuntimeException(e);
        } finally {
