...
import static com.hcop.ptn.constants.ConfigKey.CONFIGURATION_PATH;
...
public class SyncOracleTnsNames {
	...
	private void initAllFile(Map<String, String> omsConfMap){
		...
		try {
			getPktConfig2Map(omsConfMap);
		} catch (Exception e) {
			log.error("pkt's file 'tnsnames.ora' get error",e);
			msg +="read pkt oracle config error!\r\n";
		}
	}

	private void getPktConfig2Map(Map<String, String> omsConfMap) throws Exception {
		String[] pktInstances = omsConfMap.get("PKT_INSTANCE").split(",");
		for(int i = 0; i < pktInstances.length; i++) {
			String instance = pktInstances[i];
			if(omsConfMap.containsKey("PKT_"+instance+"_IP")) {
				String pktPassword = omsConfMap.get("PKT_"+instance+"_PASSWORD");
				String pktUser = omsConfMap.get("PKT_"+instance+"_USER");
				String pktIp = omsConfMap.get("PKT_"+instance+"_IP");
				SshConnect sshSession = new SshConnect(pktIp, pktUser, pktPassword);
				sshSession.login();

				String cmd = "cat /usr/Systems/PKT_" + instance + "/ORACLE/network/admin/tnsnames.ora | grep -v '#'";
				...
				saveMap2File(values, splitFileStr("pkt_" + instance));
			}
		}
	}
	...
	private void getOtnConfig2Map(Map<String, String> omsConfMap) throws Exception {
		...
		for(int i = 0; i < otnInstances.length; i++) {
			...
			//String otnName = Integer.parseInt(omsConfMap.get("OMS_VERSION")) >= 14 ? "OTNE" : "OTN";
			String cmd = "cat /usr/Systems/" + name + "_" + instance + "/ORACLE/network/admin/tnsnames.ora | grep -v '#'";
	...
	private  void getEmlConfig2Map(Map<String, String> omsConfMap)throws Exception{
		...
		for(int i = 0; i < emlInstances.length; i++) {
			...
			if(omsConfMap.containsKey("EML_"+instance+"_IP")) {
				...
				//String emlName = this.otnIp.equals(emlIp) ? "OTNE" : "EML";
				String cmd = "cat /usr/Systems/EML_" + instance + "/ORACLE/network/admin/tnsnames.ora | grep -v '#'";
	...
	private  Map<String,String> analysisStr(String str,String type){
		...
		for(int i = 1;i<values.length;i++){
			...
			//ANALOG,SDH
			if(values[i].startsWith("BMML") ||
					values[i].startsWith("WDM") ||
	...
	private  void saveMap2File(List<String> list,String fileName){
		...
		File currentDri = new File(System.getProperty(CONFIGURATION_PATH) == null ? System.getenv(CONFIGURATION_PATH) : System.getProperty(CONFIGURATION_PATH));
