...
import com.hcop.ptn.common.omsinfo.dynabean.EqualableDynaBeanClass;
import com.hcop.ptn.common.properties.Conf;
import com.hcop.ptn.common.properties.OmsBaseInfoConf;
import com.hcop.ptn.common.restful.RestFulConstant;
...
public class SyncOmsInfo {
    ...
    private static DynaClass instanceClazz;
    ...
    private static Map<String, String> confMap = Conf.instance().getPropertiesMap();
    ...
    private static void saveFile(Collection<DynaBean> instanceData) {
        ...
        StringBuilder pktInstances = new StringBuilder();
        ...
        for(DynaBean bean : instanceData) {
            ...
            if(bean.get("dir").toString().contains("PKT")) {
                pktInstances.append(bean.get("instanceNum").toString());
                pktInstances.append(",");
            }
            ...
            if(usmIp.equals(bean.get("ipAddress").toString())) {
                sb.append(usmUser);
                sb.append(lineSeparator);
        ...
        sb.append("PKT_INSTANCE=");
        if(pktInstances.length() > 0) {
            sb.append(pktInstances.substring(0, pktInstances.lastIndexOf(",")));
        }

        String omsFile = confMap.get("oms.path");
...
//		/usr/Systems/EML_1/conf/NameServerFile.nr6;
        ...
        String csgFile = confMap.get("csgConfFile");
        String snaFile = confMap.get("snaConfFile");
        String gthFile = confMap.get("gthConfFile");
    ...
    private static void generateFile(String content, String fileName) {
        ...
        String folderPath = currentDir.getAbsolutePath() + File.separator + fileName;
        ...
        try {
            if(args.length > 0) {
                if ("true".equalsIgnoreCase(args[0])) {
                    OmsBaseInfoConf omsBaseInfoConf = OmsBaseInfoConf.instance();
                    SyncOmsInfo.loadConf(omsBaseInfoConf.getProperty("IP"), omsBaseInfoConf.getProperty("USER"), omsBaseInfoConf.getProperty("PASSWORD"));
                }
