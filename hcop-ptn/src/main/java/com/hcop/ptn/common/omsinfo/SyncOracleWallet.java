...
import com.hcop.ptn.common.sftp.FtpFile;
import com.hcop.ptn.common.sftp.SFtpFileImpl;
...
public class SyncOracleWallet {
    ...
    public String downloadWalletByOms(){
        ...
        //get otn cwallet
        String[] otnInstances = omsConfMap.get("OTN_INSTANCE").equals("") ? new String[]{} : omsConfMap.get("OTN_INSTANCE").split(",");
        ...
        //get eml cwallet
        String[] emlInstances = omsConfMap.get("EML_INSTANCE").equals("") ? new String[]{} : omsConfMap.get("EML_INSTANCE").split(",");
        ...
        //get pkt cwallet
        String[] pktInstances = omsConfMap.get("PKT_INSTANCE").equals("") ? new String[]{} : omsConfMap.get("PKT_INSTANCE").split(",");
        for(int i = 0; i < pktInstances.length; i++) {
            String instance = pktInstances[i];
            if(omsConfMap.containsKey("PKT_"+instance+"_IP")){
                String ip = omsConfMap.get("PKT_"+instance+"_IP");
                String userName = omsConfMap.get("PKT_"+instance+"_USER");
                String password = omsConfMap.get("PKT_"+instance+"_PASSWORD");
                int version = Integer.parseInt(omsConfMap.get("OMS_VERSION"));

                DownLoadTask pktTask = new DownLoadTask(ip, userName, password, PKT, Integer.parseInt(instance), version, this.otnIp);
                futures.add(exec.submit(pktTask));
            }
        }
    ...
    class DownLoadTask implements Callable<Map<String, Boolean>>{
		...
		public DownLoadTask(String host, String userName, String password, String dbName, int inst, int version, String ontIp){
    		this.host = host;
