...
public class TL1Format
{
	...
	{
		return StringJoiner.join(MsgDelimiter.POSITIONAL_PARAM_DELIMITER, positionalParamList);
	}

	public static String formatKeywordParam(Map<String, String> keywordParam)
	{
		return StringJoiner.join(MsgDelimiter.KEYWORD_PARAM_DELIMITER,
				new IStringFormat<Entry<String, String>>()
				{
					public void format(StringBuilder output, Entry<String, String> s)
					{
						output.append(s.getKey()).append(MsgDelimiter.EQUAL).append(s.getValue());
					}
	...
	public static String formatTL1Request(TL1Request request)
	{
		...
		StringJoiner.join(output, MsgDelimiter.PART_DELIMITER, request.getVerb(), EMPTY_TID,
				formatAid(request.getAidList(), MsgDelimiter.AID_JOINT), EMPTY_CTAG,
		...
		for (int i = 0; i < 4; i++)
		{
			if (MsgDelimiter.PART_DELIMITER.equals(output.substring(output.length() - 1))) output
					.setLength(output.length() - 1);
			else break;
		}
		return output.append(MsgDelimiter.REQUEST_END).toString();
	}

	public static String formatTL1MessageData(TL1MessageData responseData)
	{
		StringBuilder output = new StringBuilder();
		StringJoiner.join(output, MsgDelimiter.PART_DELIMITER,
				formatAid(responseData.getAidList(), MsgDelimiter.AID_DELIMITER),
