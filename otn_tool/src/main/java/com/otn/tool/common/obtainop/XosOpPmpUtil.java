...
import com.otn.tool.common.internal.xos.response.extractors.GetPmDataExtractor;
...
public class XosOpPmpUtil {
    ...
    public static OpticalPower getOPByTp(int groupId, int emlId, Ne ne, Tp tp) {

        List<PMP> pmpList = XosPmpService.getAllPMP(groupId, emlId, ne.getName(), tp.getName(), "", "");

        PMP[] pmpStrs = getPMPByPortName(pmpList, tp.getName());
    ...
    private static PMP[] getPMPByPortName(List<PMP> pmps, String portName) {
        ...
        for (PMP pmp : pmps) {
            ...
            if ((pmp.getPtpName().equals(portName))
                    ...
                    && (pmp.getLocation() == Location.PML_NEAR_END)) {
                if (pmp.getDirection() == Direction.PMD_RVC) {
                    pmpStrs[0] = pmp;
                } else if (pmp.getDirection() == Direction.PMD_TRMT) {
                    pmpStrs[1] = pmp;
                }
            }
        }
